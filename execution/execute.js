...
var _promiseReduce = _interopRequireDefault(
  require("../jsutils/promiseReduce")
);

var _promiseForObject = _interopRequireDefault(
  require("../jsutils/promiseForObject")
);
...
function resolveField(exeContext, parentType, source, fieldNodes, path) {
  ...
  var result = resolveFieldValueOrError(
    exeContext,
    fieldDef,
    fieldNodes,
    resolveFn,
...
function resolveFieldValueOrError(
  ...
  try {
    ...
    return (0, _isPromise.default)(result)
      ? result.then(undefined, asErrorInstance)
      : result;
  } catch (error) {
...
function completeListValue(
  ...
  (0, _iterall.forEach)(result, function (item, index) {
    ...
    var completedItem = completeValueCatchingError(
      exeContext,
      itemType,
      fieldNodes,
      info,
    ...
    throw new _GraphQLError.GraphQLError(
      "Abstract type "
        ...
        "value "
          ...
          .concat((0, _inspect.default)(runtimeType), '". ') +
        "Either the ".concat(
          returnType.name,
...
function completeObjectValue(
  ...
  if (
    parsedResult &&
    returnType.pathToFields &&
    parsedResult[returnType.pathToFields] !== undefined
  ) {
    parsedResult = parsedResult[returnType.pathToFields];
  }
  ...
  if (returnType.isTypeOf) {
    ...
    if ((0, _isPromise.default)(isTypeOf)) {
      return isTypeOf.then(function (resolvedIsTypeOf) {
        if (!resolvedIsTypeOf) {
          throw invalidReturnTypeError(returnType, parsedResult, fieldNodes);
        }
